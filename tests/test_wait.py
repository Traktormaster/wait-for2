...
async def resource_worker(wait_for_impl, allow_special_raise=False, **wait_for_kwargs):
    try:
        resource = await wait_for_impl(create_resource(), timeout=0.1, **wait_for_kwargs)
    except wait_for2.CancelledWithResultError as e:
        assert allow_special_raise, "special raise is not allowed for run"
...
async def test_resource_leakage():
    ...
    await _resource_handling_test(wait_for2.wait_for, cancel_handler=cleanup_resource)
    await _resource_handling_test(wait_for2.wait_for_special_raise, allow_special_raise=True)

    with pytest.raises(AssertionError, match="wait_for within a task ignored the cancellation|resources were leaked"):
        await _resource_handling_test(wait_for2.wait_for_no_waiter)

    with pytest.raises(AssertionError, match="wait_for within a task ignored the cancellation|resources were leaked"):
        await _resource_handling_test(wait_for2.wait_for_no_waiter_alt)
